...
async function notifyAdminAboutPayment(userId, type, amount) {
  ...
  const typeText = {
    ...
    'renew_with_penalty_light': 'Восстановление со штрафом (льготный период)',
    'renew_with_penalty_heavy': 'Восстановление со штрафом (просрочка >5 дней)'
  }[type] || 'Оплата';
...
async function approvePayment(userId, type) {
  ...
  switch (type) {
    ...
    case 'renew_with_penalty_light':
    case 'renew_with_penalty_heavy':
      newEndDate = await UserModel.extendSubscription(userId, subscriptionDays, true);
      const penaltyType = type === 'renew_with_penalty_light' ? 'льготный период' : 'просрочка >5 дней';
      message = `⚠️ Подписка восстановлена со штрафом (${penaltyType})\n\nНовая дата окончания: ${newEndDate}\n\nВ следующий раз не опаздывайте!`;
      break;
  }
