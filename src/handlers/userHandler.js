...
async function calculateTotalPrice(user) {
  ...
  // Если подписка еще активна (сегодня последний день тоже считается активным)
...
async function handlePaymentSubmit(userId, paymentType) {
  ...
  switch (paymentType) {
    ...
    case 'penalty_light':
      ...
      const price1mLight = parseFloat(prices.member_price_1m);
      amount = price1mLight + penalty1;
      type = 'renew_with_penalty_light';
      typeDisplay = `восстановление со штрафом (льготный период)`;
      break;
    ...
    case 'penalty_heavy':
      ...
      const price1mHeavy = parseFloat(prices.member_price_1m);
      amount = price1mHeavy + penalty2;
      type = 'renew_with_penalty_heavy';
      typeDisplay = `восстановление со штрафом (просрочка >5 дней)`;
      break;
