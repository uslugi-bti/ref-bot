...
class UserModel {
  ...
  // Получить всех пользователей, которые должны быть кикнуты (просрочка или статус kicked)
  static async getUsersToSync() {
    const db = getDb();
    return await db.all(`
      SELECT * FROM users 
      WHERE (subscription_end < date('now') OR status = 'kicked')
        AND id NOT IN (SELECT id FROM users WHERE status = 'active' AND subscription_end >= date('now'))
    `);
  }
