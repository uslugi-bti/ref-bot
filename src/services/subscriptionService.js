...
function startSubscriptionCheck(bot) {
  ...
  const cronTime = process.env.CHECK_CRON || '30 8 * * *';
  
  cronJob = cron.schedule(cronTime, async () => {
    console.log('🔍 Running subscription check...');
    await checkExpiringSoon(bot);
    await checkExpiredAndKick(bot);
    await syncChannelMembership(bot);  // ← НОВАЯ ФУНКЦИЯ СИНХРОНИЗАЦИИ
  });
...
async function checkExpiredAndKick(bot) {
  ...
  for (const user of expiredUsers) {
    ...
    try {
      ...
      if (daysOverdue > 5) {
        await UserModel.setPenalty(user.id, 30);
      }
      ...
      if (daysOverdue <= 5) {
        ...
        message = 
          ...
          `Теперь стоимость продления составляет ${prices.penalty_price_2} USDT (фиксированный штраф).\n\n` +
...
// НОВАЯ ФУНКЦИЯ: синхронизация статуса участников канала
async function syncChannelMembership(bot) {
  console.log('🔄 Starting channel membership sync...');
  
  // Получаем пользователей, которые НЕ должны быть в канале
  const usersToCheck = await UserModel.getUsersToSync();
  ...
  for (const user of usersToCheck) {
    try {
      // Проверяем, действительно ли пользователь в канале
      const isActuallyMember = await telegramService.isMember(user.id);
      checkedCount++;
      
      if (isActuallyMember) {
        console.log(`🔨 SYNC: User ${user.id} (${user.first_name || 'no name'}) has expired/kicked status but still in channel - kicking...`);
        await telegramService.kickUser(user.id);
        kickedCount++;
        ...
        try {
          await telegramService.sendMessage(
            user.id,
            `❌ Ваша подписка истекла или была отключена, вы исключены из канала.\n\nДля восстановления нажмите /start`
          );
        } catch (msgError) {
          // Если не отправилось - игнорируем
        }
  ...
  console.log(`✅ Sync completed: checked ${checkedCount} users, kicked ${kickedCount} users`);
}
