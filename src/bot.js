...
async function setupBotCommands(bot) {
  ...
  bot.help(async (ctx) => {
    ...
    if (isAdmin) {
      helpText += `\n\n👑 АДМИН-КОМАНДЫ:\n• /start - Админ-панель\n• /testkick ID - Принудительный кик пользователя из канала`;
    }
    
    await ctx.reply(helpText);
  });

  // КОМАНДА ДЛЯ ТЕСТА КИКА (только админ)
  bot.command('testkick', async (ctx) => {
    ...
    if (!userId) {
      await ctx.reply('❌ Укажи ID пользователя: /testkick 123456789');
      return;
    }
    
    const testTelegramService = new TelegramService(ctx);
    
    try {
      const isMember = await testTelegramService.isMember(userId);
      if (!isMember) {
        await ctx.reply(`ℹ️ Пользователь ${userId} уже не в канале`);
        return;
      }
      
      const result = await testTelegramService.kickUser(userId);
      if (result) {
        await ctx.reply(`✅ Пользователь ${userId} кикнут из канала`);
        await UserModel.kickUser(userId);
      } else {
        await ctx.reply(`❌ Не удалось кикнуть ${userId}. Проверь:\n1. Бот админ канала\n2. Права на бан есть\n3. GROUP_CHAT_ID правильный`);
      }
    } catch (error) {
